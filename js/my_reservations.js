...
let ddt = {reservation_id: 0, created_at: 1, service: 2, subservice: 3, call_method: 4, payment_method: 5, service_fee: 6,  appointment_date: 7, appointment_time: 8, service_description: 9, updated_at: 10, modify_count: 11};
...
if($('#myTable').length){
	...
	"serverSide": true,
	"ajax": {
		url: "../connect/user_main.php?ch=get_data",
		data: function(d){
		} 
	},
	"bSortCellsTop" : true,
	...
	initComplete : function(setting, json){ //When table has been fully initialize
		...
		columns: [
			{ data: ddt.created_at, className: "editor_btn text-center", render: function(data, type, row, meta){  return data + ( (row[ddt.modify_count] == 0)? '' : '<a onclick="open_changes(event)" href="#" style="background-color:#990; color:#FFF !important; display:inline-block; padding: 0px 5px; border-radius:7px;  marging-left: 10px; text-decoration:none" ><i class="fa fa-newspaper-o"></i> '+row[ddt.modify_count]+'</a><br><br>' )+ '<div><a  target="_blank" href="./edit-reservation.php?action=reschedule&reservation_id='+row[0]+'" class="btn btn-xs pentry_update btn-warning" onclick="edit_prow(event)"><span class="fa fa-spinner"></span> Reschedule</a><a target="_blank" href="./edit-reservation.php?action=modify&reservation_id='+row[0]+'" class="editor_update"><span class="fa fa-edit"></span> Modify </a> | <button onclick="remove_entry(event, 0)" class="editor_remove"><span class="fa fa-trash-o"> Cancel</button></div>'; }},
			{ data: ddt.service},
...
function open_changes(event){
	event.preventDefault();
	 ...
	 $("#modal_changes .modal-body tbody").html('<tr><td colspan="3" class="text-center py-5"><span class="fa fa-spin fa-spinner fa-5x"></span></td></tr>');
	  ...
	  $.ajax({
	  type: "GET",
	  url:  "../connect/user_main.php?ch=reservation_changes&reservation_id="+reservation_id,
	  success: function(data){
